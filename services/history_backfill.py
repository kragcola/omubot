"""Core connection-stage backfill for recent group history."""
...
def _contains_debug_command(segments: list[dict[str, Any]]) -> bool:
    ...
    for seg in segments:
        text = str(seg.get("data", {}).get("text", "") or "")
        if re.match(r"^/debug(?:\s|$)", text.lstrip(), flags=re.IGNORECASE):
            return True
    return False
...
async def run_history_backfill(
    ...
    """Backfill recent allowed-group messages before plugin connection hooks."""
    if group_ids is None:
        group_list = await bot.get_group_list()
        resolved_group_ids = [str(group["group_id"]) for group in group_list]
    else:
        resolved_group_ids = list(group_ids)
    allowed = getattr(ctx, "allowed_groups", set())
    if allowed:
        resolved_group_ids = [
            group_id for group_id in resolved_group_ids if int(group_id) in allowed
        ]
    ...
    if group_cfg is not None and hasattr(group_cfg, "resolve"):
        for group_id in resolved_group_ids:
            try:
                resolved = group_cfg.resolve(int(group_id))
            except Exception:
                continue
            if resolved is None:
                continue
            ...
            if str(getattr(resolved, "sticker_mode", "inherit") or "inherit") == "off":
                continue
            learn_sticker_groups.add(group_id)

    _L.info("loading history | groups={}", len(resolved_group_ids))
    counts = {
        group_id: ctx.config.group.resolve(int(group_id)).history_load_count
        for group_id in resolved_group_ids
    }
