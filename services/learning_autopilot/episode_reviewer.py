...
class EpisodeAIReviewer:
    ...
    async def run_one_batch(
        ...
        async with aiosqlite.connect(self._db_path) as db:
            ...
            if not state.get("active"):
                cur = await db.execute("SELECT COUNT(*) FROM episodes WHERE episode_state = 'candidate'")
            ...
            if not rows:
                ...
                await self._save_meta(db, _STATE_KEY, state)
                cur2 = await db.execute("SELECT COUNT(*) FROM episodes WHERE episode_state = 'candidate'")
    ...
    async def _apply_verdict(
        ...
        await db.execute(
            "UPDATE episodes SET episode_state = ?, meta_json = ?, updated_at = ? WHERE episode_id = ?",
    ...
    async def _save_meta(self, db: aiosqlite.Connection, key: str, value: Any) -> None:
        try:
            await db.execute("CREATE TABLE IF NOT EXISTS episode_meta (key TEXT PRIMARY KEY, value TEXT)")
            await db.execute(
