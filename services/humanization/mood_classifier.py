...
class MoodClassifier:
    ...
    async def classify(self, messages: list[dict[str, Any]]) -> MoodDecision:
        signals = _signals(messages[-_MAX_WINDOW:])
...
def _signals(messages: list[dict[str, Any]]) -> MoodSignals:
    ...
    sticker_density = sum(1 for text in texts if _has_any(text, _STICKER_TOKENS)) / len(texts)
