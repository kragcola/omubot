...
| 2026-05-23 | 信息速递 双卡仪表盘底排 | ✅ 落地 | `AllOverviewDashboard.vue` 底排引入 `<section class="ov-bottom">` 7fr/5fr 双栏 —— 左 Live Feed + 右「群活跃榜」并列；Feed row 从 52px 双行改为 36px 单行（dot/title/meta/chip/time 5 列 inline，title `max-width: 18ch` 限宽避免空白）；新增 `groupRanking` computed 按 group_id 聚合 props.items 取 top 8（序号 1/2/3 warn/info/success 着色 + 6px 横向 bar 按 topNoun 着色 + count）；≤1100px 双栏堆叠回单列；vue-tsc 0 errors；build OK，LearningView chunk **158.67 KB（gzip 45.62 KB，相对词条版 +1.47 KB ≈ rank computed + bar 样式）** |：行高 52px，padding 10px 14px，三槽 grid `8px / 1fr / auto` —— 左侧 8px 圆点（statusTone 着色） + 中部主从行（标题 14/500 ellipsis + 元信息 12/400 dot-separated `状态·群·置信%`） + 右侧栈（noun chip 11px 按 noun 着色 + 相对时间 11.5/400 tabular-nums）；新增 `formatRelativeTime()`（刚刚 / N 分钟前 / N 小时前 / 昨天 / N 天前 / MM-DD）替代绝对时间；新增 `nounToneOf()` 表（slang/style/graph_relation neutral · episode info · memory success · fact warn）；移除 `feed-row:nth-child(even)` 双列 dashed 分隔与 `:nth-last-child(-n+2)` 边界处理；mobile 仍单列，<720px 隐藏 noun chip；vue-tsc 0 errors；build OK，LearningView chunk 157.20 KB（gzip 45.07 KB，相对仪表盘版 +0.87 KB 纯样式与时间助手） |
