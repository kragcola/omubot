...
| **P2.12** | 5 | ✅ | 自主验收通过：`services/sticker/fairmatch.py` 40 行；调用频次直方图对过度集中 id 降权 0.5，provider 可选接入；5 条新测试 |
| **P2.13** | 5 | ✅ | 自主验收通过：`services/url_meta/video_adapter.py` 110 行；Bilibili BV/av 与 YouTube watch/shorts/youtu.be 专用 metadata adapter 默认禁用，5 条新测试 |
| **P2.7+P3.5+v2-灰度** | 6 | 🟡 | 2026-05-26 03:04 CST 进入灰度：993065015 启全部 7 个已接线 humanization 旗标（context_providers / register_classifier / sticker_register_provider / thinker_provider / rewrite_threshold=0.4 / semantic_gate_dynamic / kaomoji_enforce_strict）+ 已落地无旗标 P2.5/P2.11/P3.3/P3.8/P3.9；984198159 自动走 v1 对照（不在 humanization.runtime_groups 内）；persona_v2 双群保持 shadow_compare；Wave 2-5 standalone 模块（binary_planner / topic_drift / mood_classifier / affection_classifier / sticker_decision_provider / coupling / fairmatch / video_adapter）按 Part 6 范围**未接入 ChatPlugin / scheduler**——本次灰度只覆盖已接线的 v2 链路。证据：维护日志 2026-05-26 条目（D4 三元组完整）；24h 后跑 §4 16 项矩阵 |
...
### P2.12 领单拆分（执行前）— Codex / 2026-05-26 09:15 CST

- **任务边界**：新增 `services/sticker/fairmatch.py` 的 long-tail rerank 纯函数，并让 `StickerDecisionProvider.decide()` 可选接收调用频次直方图；不改 `StickerStore` schema，不新增持久化统计表，不改变未传 histogram 时的现有候选顺序。
- **自主评估**：当前 `StickerDecisionProvider` 已把四类候选收敛到单点，但候选池只 dedupe/cap，不感知“某些 sticker_id 近期被过度使用”。P2.12 可先提供可选 rerank：当调用方传入 `usage_counts` 时，对过度集中 id 施加 0.5 权重并稳定后置；不传则完全 no-op。
- **执行拆分**：
  1. 新建 `services/sticker/fairmatch.py` ≤ 60 行，定义 `fairmatch_weights()` 与 `fairmatch_rerank()`；过度集中 id 权重为 0.5，其余为 1.0。
  2. `StickerDecisionProvider.decide()` 增加可选 `usage_counts` 参数，候选 dedupe 后调用 `fairmatch_rerank()`，不改变 `send_probability` 与 gate。
  3. `services/sticker/__init__.py` 导出 FairMatch helper。
  4. 新增 `tests/test_fairmatch.py` ≥ 4，覆盖权重、稳定后置、no histogram no-op 与 provider 接线。
- **风险与回滚**：纯排序逻辑，无 I/O / RuntimeStateBus 写入；最大风险是高优先级 tool_call sticker 被后置。回滚为删除 `fairmatch.py` / 新测试，撤销 decision_provider 可选参数与 §6 / §9 P2.12 回填。

### P2.12 完成记录（执行者 Codex）— 2026-05-26 09:17 CST

自验结果：P2.12 完成并自主验收 ✅。本步新增 FairMatch long-tail rerank，并以 `usage_counts` 可选参数接到 `StickerDecisionProvider`；调用方不传 histogram 时现有候选顺序、概率与 gate 完全保持不变。

- `services/sticker/fairmatch.py`：40 行，新增 `fairmatch_weights()` 与 `fairmatch_rerank()`；当某个 sticker_id 在调用频次直方图中占比 ≥ 0.5 时，权重降为 0.5，并在候选池内稳定后置。
- `services/sticker/decision_provider.py`：`decide()` 增加可选 `usage_counts`，候选 `_dedupe()` 后调用 `fairmatch_rerank()`；不改 `_send_probability()` / gate / cooldown / density feedback。
- `services/sticker/__init__.py`：导出 `fairmatch_weights` / `fairmatch_rerank`。
- `tests/test_fairmatch.py`：新增 5 条测试，覆盖权重降级、稳定后置、空 histogram no-op、零值 histogram 与 provider 接线。

验证：

- `source ./scripts/dev/env.sh && uv run pytest -q tests/test_fairmatch.py tests/test_sticker_decision_provider.py tests/test_sticker_density_feedback.py` → `23 passed`。
- `source ./scripts/dev/env.sh && uv run ruff check services/sticker/fairmatch.py services/sticker/decision_provider.py services/sticker/__init__.py tests/test_fairmatch.py tests/test_sticker_decision_provider.py` → passed。
- `source ./scripts/dev/env.sh && uv run pyright services/sticker/fairmatch.py services/sticker/decision_provider.py services/sticker/__init__.py tests/test_fairmatch.py` → `0 errors, 0 warnings, 0 informations`。
- `source ./scripts/dev/env.sh && uv run python -m py_compile services/sticker/fairmatch.py services/sticker/decision_provider.py services/sticker/__init__.py tests/test_fairmatch.py` → passed。
- `wc -l services/sticker/fairmatch.py` → `40`。
- `source ./scripts/dev/env.sh && uv run pytest --collect-only -q` → `1873 tests collected`。
- D1 grep：`rg -n "fairmatch_weights|fairmatch_rerank|usage_counts|test_fairmatch" services/sticker tests/test_fairmatch.py docs/tracking/omubot-humanization-part2-3-execution.md` → 命中 FairMatch helper、provider 可选接线、新测试与本追踪记录。

D2 / 回滚：本任务没有 I/O、持久化写入或 cancel-path；`usage_counts=None` 是完全 no-op。回滚为删除 `services/sticker/fairmatch.py` / `tests/test_fairmatch.py`，撤销 `decision_provider.py` 的 `usage_counts` 参数与 `services/sticker/__init__.py` 导出，并撤销 §6 / §9 的 P2.12 回填。

### P2.13 领单拆分（执行前）— Codex / 2026-05-26 09:26 CST

- **任务边界**：新增 `services/url_meta/video_adapter.py` 的 Bilibili / YouTube 专用视频元信息 adapter；默认不启用，不接生产 `PromptBuilder`，不改 `plugins/bilibili` 的拦截、封面识别、兴趣评分或 scheduler 触发链路。
- **自主评估**：P2.11 已提供通用 og:title 抓取，但视频站点需要专用 API/URL 解析以避免把长页面正文或动态脚本页误当上下文。本步先做显式 `enabled=True` 才工作的轻量 adapter，调用方未来可在灰度配置里选择是否把 `【视频信息】` block 接入 prompt。
- **执行拆分**：
  1. 新建 `services/url_meta/video_adapter.py` ≤ 110 行，定义 `VideoMetadata` 与 `collect_video_metadata()`。
  2. 覆盖 Bilibili `BV` / `av` 与 YouTube `watch` / `shorts` / `youtu.be` URL 识别；仅访问平台专用 metadata endpoint，失败静默返回空。
  3. `services/url_meta/__init__.py` 导出 video adapter 入口，但保持默认 no-op：`enabled=False` 不解析、不发网络。
  4. 新增 `tests/test_video_adapter.py` ≥ 5，覆盖 Bilibili、YouTube、非视频 URL、默认禁用 no-op 与 fetch 失败静默。
- **风险与回滚**：主要风险是引入额外网络调用或和现有 B 站插件重复触发；因此本步默认关闭且不接生产链路。回滚为删除 `video_adapter.py` / 新测试，撤销 `services/url_meta/__init__.py` 导出与 §6 / §9 P2.13 回填。

### P2.13 完成记录（执行者 Codex）— 2026-05-26 09:35 CST

自验结果：P2.13 完成并自主验收 ✅。本步新增独立视频 metadata adapter，调用方必须显式传 `enabled=True` 才会解析 Bilibili / YouTube URL 并访问平台 metadata endpoint；默认禁用时完全 no-op，且不接 `PromptBuilder` / `plugins/bilibili` 生产链路。

- `services/url_meta/video_adapter.py`：110 行，新增 `VideoMetadata` 与 `collect_video_metadata()`；识别 Bilibili `/video/BV...` / `/video/av...`、YouTube `watch?v=` / `shorts/` / `youtu.be/`，分别访问 Bilibili `x/web-interface/view` 与 YouTube oEmbed，失败或非 200 静默返回空。
- `services/url_meta/__init__.py`：导出 `VideoMetadata` / `collect_video_metadata`，保留 P2.11 `UrlTitle` / `collect_url_titles` / `build_url_title_context` 入口。
- `tests/test_video_adapter.py`：新增 5 条测试，覆盖默认禁用 no-op、Bilibili BV/av、YouTube watch/shorts/youtu.be、非视频 URL 跳过与 fetch failure 静默。

验证：

- `source ./scripts/dev/env.sh && uv run pytest -q tests/test_video_adapter.py tests/test_og_title.py` → `14 passed`。
- `source ./scripts/dev/env.sh && uv run ruff check services/url_meta/video_adapter.py services/url_meta/__init__.py tests/test_video_adapter.py` → passed。
- `source ./scripts/dev/env.sh && uv run pyright services/url_meta/video_adapter.py services/url_meta/__init__.py tests/test_video_adapter.py` → `0 errors, 0 warnings, 0 informations`。
- `source ./scripts/dev/env.sh && uv run python -m py_compile services/url_meta/video_adapter.py services/url_meta/__init__.py tests/test_video_adapter.py` → passed。
- `wc -l services/url_meta/video_adapter.py` → `110`。
- `source ./scripts/dev/env.sh && uv run pytest --collect-only -q` → `1878 tests collected`。
- D1 grep：`rg -n "VideoMetadata|collect_video_metadata|test_video_adapter|P2\\.13" services/url_meta tests/test_video_adapter.py docs/tracking/omubot-humanization-part2-3-execution.md` → 命中新 adapter、导出、新测试与本追踪记录；未命中 PromptBuilder 或 Bilibili plugin 生产接线。
- `git diff --check -- docs/tracking/omubot-humanization-part2-3-execution.md services/url_meta/video_adapter.py services/url_meta/__init__.py tests/test_video_adapter.py` → passed。

D2 / 回滚：本任务没有生产接线、持久化写入或 cancel-path；`enabled=False` 默认不解析、不发网络。回滚为删除 `services/url_meta/video_adapter.py` / `tests/test_video_adapter.py`，撤销 `services/url_meta/__init__.py` video adapter 导出，并撤销 §6 / §9 的 P2.13 回填。
