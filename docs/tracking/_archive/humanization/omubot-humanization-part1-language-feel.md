...
| **Part 5** | 回复分段重构（"自然不打断" — 取消硬拆 + 概率合并 + 标点保留） | 与 Part 1 U1（segmentation 双实现合并）正交；详见 [part5 doc](./omubot-humanization-part5-segmentation.md)。**2026-05-27 状态**：P5.1~P5.4 ✅；P5.5 代码已落地（默认 `True` + `_legacy_segment_path` 删除）但**误判验收已撤回**（🟡 ⚠️），原因是 P5 fallback 路径与 [Part 6 bugfix](./omubot-humanization-part6-bugfix-part1.md) §1.2 故障强耦合 + balanced 实测单段死锁证伪 P5 默认 True 真正分段；P5.5/P5.6 阻塞于 bugfix Phase 1 全绿。U1 已合并的 segmentation.py 现是 Part 5 算法主路径基线 |
