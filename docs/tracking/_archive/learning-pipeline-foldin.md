...
| 2026-05-23 | 4 noun 主面板美学统一（以黑话为模板） | ✅ 落地 | 用户原话「美学统一：已黑话界面为模板，将界面风格统一到管线内全部页面。包括颜色衬底，翻页，词条间隔，词条配置等」—— 前序 PR 已对齐 stage→mode 路由语义和 SlangTermList / LearningTable 行级视觉契约，但 4 个 noun 主面板的「壳」和「翻页/配置」交互仍未统一：slang 是模板（AppPanelSection eyebrow=Review Queue + 顶/底 NPagination + 行尾配置 pill + 8px gap + 10/14/10/18 padding + 3px 状态色侧栏），memory（LearningTable）裸 div + 「加载更多」按钮，style 裸 section + 一次 80 条铺平没有翻页，episode 直接 NDataTable 跟其他 3 个 noun 视觉语言完全不同源。实施：① **LearningTable.vue** 包 `AppPanelSection`（title 由 LearningView 通过新 prop 注入），客户端分页 PAGE_SIZE=20、`pageCount = Math.ceil(items/20) + (hasMore ? 1 : 0)`，翻到末页且 hasMore 时 watch 触发 `emit('loadMore')` —— 既保持 slang 同款 NPagination 视觉又不破坏后端「加载更多」分批契约；行 grid 7 列扩到 8 列，新增「配置」pill（status 与 actions 之间，与 SlangTermList `slang-term-row__config` 完全同源：`color-mix(--om-info 6%)` 衬底 + 18px 圆角 + 20px 圆形图标背景 hover 反相）；底部 NPagination(page-slot=7)。② **LearningView.vue** 删 `<header.learning-items__header>` 三层装饰（含一份重复 `learning-snapshot__eyebrow`）和 27 行无消费者 CSS。③ **StyleMainPane.vue** 包 `AppPanelSection eyebrow=Style Expressions` + 客户端分页 PAGE_SIZE=12（后端 limit 80 一次拉满，前端切片）；卡片 padding 14/16/14/20→12/14/12/18、list gap 10→8px、h3 margin 10/0/6→8/0/4、meta 12→11.5px / padding-top 8→6px 全部向 slang 看齐；新增 `expression-item__rail` 列把「配置」pill 抽出来与 5 段 NButton 竖排（≤1100px 横排）；顶/底 NPagination。④ **EpisodeMainPane.vue** 整体重写：删除 NDataTable + 4 列 columns + 自带分页，改成行卡列表（10/14/10/18 padding + 10px 圆角 + 3px 状态色侧栏 + hover translateY(-1px) + shadow-sm + border-strong + 与 slang 同源）；行 grid 8 列 `situation/group/time/50px conf/配置 pill/decay/status/actions`；status 文字按 `episodeTone()` 着色（success/pending/rejected/neutral），actions hover 浮出（22px ghost 按钮 + success/danger 描边变体）；客户端分页 PAGE_SIZE=20 + 顶/底 NPagination；操作按钮收敛 candidate→批准+停用 / approved+enabled_for_prompt→停用 / disabled→恢复（`openActionDialog` 契约 0 改动）。约束：4 个 noun 的 FoldInProvider Teleport / props / state.ts / composable / API 调用 0 改动，slang `SlangTermList` 自身 0 改动（它就是模板），漂移视图（queueMode='drift'）仍走 `SlangDriftCard` 不纳入 Episodes 同源；AppPanelSection `#aside` 槽与底部居中 NPagination 双布局，`pageCount<=1` 时双方 `v-if` 隐藏避免空槽。vue-tsc 0 errors；build OK，LearningView chunk **159.76 KB（gzip 46.30 KB，相对上一版 +2.93 KB ≈ 3 套 AppPanelSection 壳 + 3 套顶/底 NPagination + 3 套配置 pill + ep-row 重写 CSS）** |
