...
> mode: task-bug
> 最后更新：2026-07-15 CST
> 当前下一步：精确提交实现，bot-only build/recreate 后做容器内真实链回放与公开群零出站观察。
> 阻塞：无。
...
- deployed_baseline: nested forward 实现提交 `2027858`；image `a1792c614fec...` / container `bd58abf979f3...`。
- confirmed_gaps:
...
- [x] active/silent/off 群访问策略不变；公开群保持零出站（静态/测试已闭环，待部署窗口复核）。
...
| File | Change | Status |
| --- | --- | --- |
| `docs/tracking/ACTIVE.md` | 指向本 task-bug | in_progress |
| `docs/tracking/rich-message-context-completion-2026-07-15.md` | 合同、实验与交付台账 | in_progress |
| `services/onebot_segments.py` | 共享有界 segment/reply/embedded-forward renderer | implemented |
| `kernel/router.py` | active/private resolver + suppressed rich timeline | implemented |
| `services/history_backfill.py` | local reply index + JSON/forward + 单条隔离 + no vision | implemented |
