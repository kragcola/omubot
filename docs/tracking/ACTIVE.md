...
- status: Agent Runtime v2 默认关闭 fence release 已于 2026-08-15 上线：`ba32cdf` / image `6dc8ab9e8a30…`。本轮 bot-only 行为与 QZone ownership 修复已于 2026-08-16 上线：`d311056` / image `60f179753e94…`，交叉 199 passed、全仓 5729 passed / 17 skipped；新 bot restart=0，Admin=200，Runtime unauth=401。production TOML 无 `[agent_runtime]`，正确 preflight=`not_ready/agent_runtime_disabled`，storage 无 contract-named Runtime source/manifest/backup；release 与 `origin/main` 已对齐至 `0ee2c3e`，GitHub Typed boundaries CI 通过；NapCat 未重启或重建，ID/image/start/restart 均不变。
- next_step: 只读查找并独立核验 production source/schema、backup SHA-256、restore/rollback rehearsal、具名 operator ACL、Worldbook witness、profile-bound manifest，以及 provider cooperative-cancellation/serial-throughput canary evidence。完整 Runtime 证据前禁止启动 worker、修改 gate 或伪造任何 source/backup/manifest。
- last_completed: Runtime v2 的 profile/source、operator ACL、trusted invocation、受限组合根、lease/recovery、bootstrap 生命周期、worker hard gate、Admin operator transport、offline reconciliation、strict GET query、provider 前 owner/token execution fence、同 token extension、guard terminalization 和 cancel-safe worker stop 已以默认关闭 image 上线。日志驱动的 sticker/scheduler/schedule 修复增加 raw-CQ callback 与 failed-sticker continuation 防线；全仓扫描发现并修复 QZone services-to-plugin reverse import，保持 pre-dispatch terminal / post-dispatch unknown 语义。生产启动的实际日程模型 prose 首轮已安全 retry 并生成 14 slots；2026-08-16 artifact inventory 已确认五源/备份/演练/operator/canary/Worldbook witness 均仍为外部输入，随后提交并快进 `origin/main=0ee2c3e`、Typed boundaries CI 成功，详见 active tracker matrix。
- rollback: 将 `omubot-bot:pre-log-behavior-fix-20260815`（`6dc8ab9e8a30…`）重标为 `omubot-bot:latest`，然后仅执行 `docker compose up -d --no-deps --force-recreate --no-build bot`；不建/删 production DB、不重建 NapCat；`BUILTIN_WIRE_PROFILE.validated=false`。
