...
- status: A18 ingress-only scope repair 仍在生产：`dc67f19` / image `7ca937f05eed…`，profile 精确包含 `memory:read`、`network:search`、`time:read` 和唯一 `network:web-search` target。新候选 `af75232` 尚未部署：它把无 `SEARCH_API_KEY` 的默认 `auto` 搜索从不可取消的 `ddgs` 线程回退改为有界、`trust_env=False` 的 Bing RSS async transport。当前 worker gate 不变，17 个 activation 与 4 个 rollback gate 仍为 `not_assessed`，worker lease/run/tool/event 均为 0，dispatcher 保持 fail-closed。
- next_step: 从隔离 worktree 将 `af75232` 构建并仅替换 bot，记录 image 内的真实 provider success/cancellation/serial evidence 与无 worker/NapCat drift。之后仍须独立取得自然 post-A18 OneBot canary、registry/LLM witness 和 source-bound Worldbook `proposal -> named decision -> reducer receipt`；不得伪造 gate 或发送 QQ/QZone 测试消息。
- last_completed: A20 候选已修复 provider root cause：默认 `auto` 在无 key 时曾将 `ddgs` 放入 `asyncio.to_thread`，取消任务不能停止底层调用，且生产容器代理失效、直连 DuckDuckGo 超时。`af75232` 改用固定 `cn.bing.com` RSS 和共享 async public-HTTP boundary；RED 1 failed、相关 156 passed、完整 pytest 5781 passed / 17 skipped、Ruff/Pyright clean。受治理的真实本地 probe 解析 5 个结果块；production image rollout 尚未进行。
