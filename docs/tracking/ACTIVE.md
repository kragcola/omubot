...
- status: A18 web-search scope repair 已于 2026-08-17 CST 上线：`dc67f19` / image `7ca937f05eed…`。enabled ingress-only profile 现精确包含 `memory:read`、`network:search`、`time:read` 和唯一 `network:web-search` target；旧 manifest/source inventory 不变，新 generation manifest 已重新绑定 profile。当前 `qq-bot` restart=0、OOM=false、Admin=200、未认证 Runtime API=401；17 个 activation 与 4 个 rollback gate 仍为 `not_assessed`，worker lease/run/tool/event 均为 0，dispatcher 保持 fail-closed。
- next_step: 依次收集并独立核验真实 operator action、自然 OneBot canary 与 registry/LLM witness、provider cooperative-cancellation/serial-throughput 和 source-bound Worldbook authority。只在每项都有可审计证据、完整 manifest 更新并经新独立审查后，才启动唯一 worker；不得伪造 gate 或发送 QQ/QZone 测试消息。
- last_completed: A18 修复了 provisioner 为 `network:web-search` 漏配 `network:search` 的 capability mismatch；新 provision 均强制该依赖，既有 production profile 仅在唯一 target、全 gate `not_assessed` 时才可 repair。repair 采用目录 FD、`O_NOFOLLOW`、inode 复核、不可变 generation manifest、文件及目录 `fsync` 和最后 config 指针原子替换；两轮独立审阅先发现并关闭 TOCTOU、CLI/锁与断电耐久性 P2，最终无 P0-P3。62 个相关契约测试、Ruff、Pyright、diff clean；无网络只读 dry-run、实际 apply 和 bot-only restart 后，五源 preflight=ready、旧 manifest/source inventory 与 Runtime ledger 不变量保持。
- rollback: 代码回滚可将 `omubot-bot:pre-agent-runtime-web-search-scope-20260817`（`aed72adb25a0…`）重标为 `omubot-bot:latest` 后仅替换 bot；该 scope 增量本身不打开 worker。若需恢复旧 profile 指针，使用 `config/backups/config.before-agent-runtime-web-search-scope-20260817t003700z.json` 的精确内容原子恢复 config，保留 generation manifest 作为审计 orphan。未来 worker 回滚先将 `agent_runtime.enabled=false`，保留 `unknown`/`dispatching`。不建/删 production DB、不重建 NapCat；`BUILTIN_WIRE_PROFILE.validated=false`。
