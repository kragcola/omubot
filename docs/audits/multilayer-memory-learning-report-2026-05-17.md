...
- **A1.3** ✅ **已落地**（2026-05-21 stash 恢复 commit `3477163`）：`services/slang/store.py:1253` `_attach_normalizer` 已实现，5 个写入路径全部接入 — `create_term`（line 1393）、`upsert_ai_approved_term` 的 update branch（1494）和 create branch（1565）、`update_term` 文本/含义变更时（2351）、`merge_terms`（2572）。回归测试：`tests/test_slang_normalizer_attach.py` 5 条 case（4 正例 + 1 反例「文本无变化时跳过」）。保持 § 7.5 决议 5 的"normalizer 纯 domain"职责。来源：§ 8.2.4a + § 7.5 + 2026-05-19 基石补丁 P3。
...
| `LearningNormalizerStore.attach_candidate(domain='slang')` 在新写入路径上被调用 | ✅ green（2026-05-21 commit `3477163` 落地，5 路径接入 + 回归测试齐全） |
