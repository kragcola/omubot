...
| 预回复思考 | thinker（内置） | `true`（轻量 LLM 判断 reply/wait + retrieve_mode 四档；2026-05-22 PR5 删除 search action） |
...
| 知识库 | `/admin/knowledge` | 用户侧 2 tab：文档源 / 上下文调试（内置 命中详情 / Prompt Pack / 评测指标 三 tab，单一 query 同时驱动）；管理员维护进入"管理"按钮抽屉（候选 / 图谱关系 / 图谱节点） |
...
- 阶段 3 进行中：
  ...
  - ✅ **KnowledgeView** — 2026-05-21 完成 PR B-1 helpers / B-2 3 只读子组件 / B-3 5 交互子组件 / C 视觉收敛 4 轮拆分：2186 行 → **754 行（-65.5%）** + 8 个子组件 + 3 helper（types/formatters/badges），头部样式全部迁到 [AppPanelSection](../admin/frontend/src/components/common/AppPanelSection.vue)。PR C 把 KnowledgeMetricsPanel 的 `Sources / Types`、KnowledgeContextPanel 的 `Prompt Pack`、KnowledgeGraphPanel 的 `Entities` 共 4 块 `section-head + knowledge-eyebrow + h3` 三件套全部改 `<AppPanelSection eyebrow title>`（trailing tag 走 `#aside` slot），删冗余样式 ~109 行；`KnowledgeView-*.js` 52.82→**52.32 KB** / gzip 14.87→**14.79 KB**（与 SystemView/SlangView C 阶段同量级回吐）。同日开 **简化重构 PR1（视觉减负）+ PR2（信息架构）**：PR1 删 KnowledgeHero status-grid 6 格平铺 KPI（props 9→2）+ 新建 KnowledgeSidebar.vue（260px sticky，Index/Backlog/Actions 三段式，warn chip 点击映射到管理员 tab），主布局改 `minmax(0,1fr) 260px` 双栏 + 响应式单栏退化（与 SlangView 对齐）；PR2 删顶部 NTabs admin 三 tab（candidates/graph/graph_nodes），新建 KnowledgeAdminDrawer.vue（720px 抽屉 + 内置 NTabs，透明 wrap 现有 3 admin 子组件 props/emits 不变），KnowledgeView `#action` slot 加"管理"按钮，4 处高破坏性按钮（拒绝候选 / scope-risk 回滚 / 事实回滚 / 事实取代）全包 NPopconfirm；主视图 754→**740 行（PR2 末态）**。PR3 完成：新建 KnowledgeContextWorkspace.vue（623 行，统一 query/user/group 三输入条 + 内置 3 tab：命中详情 / Prompt Pack / 评测指标），单次 submit 并发跑 `searchKnowledge` + `debugContext`，metrics tab 内嵌"刷新"按钮；删 KnowledgeSearch.vue / KnowledgeContextPanel.vue / KnowledgeMetricsPanel.vue 三个独立子组件（共 575 行）；KnowledgeView 顶部 4 tab → **2 tab（文档源 / 上下文调试）**，加 `?tab=` 兼容跳转（search→workspace/details、context→workspace/pack、metrics→workspace/metrics、candidates|graph|graph_nodes→打开管理员 drawer 并定位 tab）；主视图 740→**755 行**，`KnowledgeView-*.js` 61.60→**63.55 KB** / gzip 17.20→**17.66 KB**（workspace 视觉收敛 +1.95 KB / +0.46 KB gzip）。**PR4 视觉收尾**（同日）：4 项 UI 打磨 — ① Sidebar Backlog 三 chip 顺序重排为 候选待审/作用域待查/跳过源 与 AdminDrawer NTabs 顺序对齐 + 每 chip 包 NTooltip 解释；② CandidatesPanel 卡片重设计（subject→predicate→object 带 ArrowForward 三段式 + 置信度 NTag 三档分级 + evidence left-border 引文 + footer dashed top-border）；③ GraphNodesPanel 4 metric tile 自研替换 MetricCard（节点/边总数 22px nowrap、主要类型 15px monospace 避免大字两行换行）+ 筛选条 panel 容器 `1.3fr 1fr 1.3fr` 三 input grid 替代 PageToolbar inline-style 220/180/220 写死宽度；④ SourcesPanel 卡片新增 preview 引文（后端 `/knowledge/sources` 加 preview 字段，取该 source 首 chunk 前 160 字符空白折叠；前端 `KnowledgeSource.preview?: string` + 3 行 line-clamp 引文样式）。附带：`.dockerignore` 加 `storage.bind-mount-snapshot-*/` 规则避免 Phase 3 named-volume 迁移本地快照阻塞 docker build。`KnowledgeView-*.js` 63.55→**66.84 KB** / gzip 17.66→**18.38 KB**（+3.29 KB / +0.72 KB gzip，4 项视觉打磨开销）。需 `docker compose up bot -d --build` 才能让 preview 字段生效。
