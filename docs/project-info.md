...
- 阶段 3 进行中：
  ...
  - ✅ **KnowledgeView** — 2026-05-21 完成 PR B-1 helpers / B-2 3 只读子组件 / B-3 5 交互子组件 / C 视觉收敛 4 轮拆分：2186 行 → **754 行（-65.5%）** + 8 个子组件 + 3 helper（types/formatters/badges），头部样式全部迁到 [AppPanelSection](../admin/frontend/src/components/common/AppPanelSection.vue)。PR C 把 KnowledgeMetricsPanel 的 `Sources / Types`、KnowledgeContextPanel 的 `Prompt Pack`、KnowledgeGraphPanel 的 `Entities` 共 4 块 `section-head + knowledge-eyebrow + h3` 三件套全部改 `<AppPanelSection eyebrow title>`（trailing tag 走 `#aside` slot），删冗余样式 ~109 行；`KnowledgeView-*.js` 52.82→**52.32 KB** / gzip 14.87→**14.79 KB**（与 SystemView/SlangView C 阶段同量级回吐）。
