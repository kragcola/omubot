...
| 知识库 | `/admin/knowledge` | 用户侧 4 tab：文档源 / 搜索 / 上下文调试 / 评测；管理员维护进入"管理"按钮抽屉（候选 / 图谱关系 / 图谱节点） |
