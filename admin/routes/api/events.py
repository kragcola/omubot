...
- ``cache_pipelines`` — per-pipeline cache-hit snapshot (every
  ``CACHE_PIPELINES_INTERVAL`` seconds, for real-time dashboard updates)
            ...
            try:
                while True:
                    ...
                    for event in group_events:
                        ...
                        elif event.get("type") == "block_trace_recorded":
                            payload = json.dumps(event, ensure_ascii=False)
                            yield f"event: block_trace\ndata: {payload}\n\n"
                    ...
                    # 4.5) Cache pipeline snapshot — pushes the same per-pipeline
                    # cache-hit data that powers the dashboard panel, so the
                    # frontend never needs a separate polling timer.
                    if usage_tracker is not None and now - last_cache_pipelines_at >= CACHE_PIPELINES_INTERVAL:
                        with contextlib.suppress(Exception):
                            from services.llm.llm_pipelines import (
                                build_cache_pipelines_payload,
                                fold_recent_into_pipelines,
                            )
                            rows = await usage_tracker.cache_hit_by_call_type(period="day")
                            recent_per_call_type = await usage_tracker.recent_calls_per_pipeline(
                                period="day", limit=5,
                            )
                            recent_overall = await usage_tracker.recent_calls_overall(
                                period="day", limit=5,
                            )
                            ...
                            yield f"event: cache_pipelines\ndata: {json.dumps(payload, ensure_ascii=False)}\n\n"
                        last_cache_pipelines_at = now
