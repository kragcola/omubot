...
def _extract_food_name(text: str) -> str:
    """Extract a compact food name from a model or local recommendation."""
    value = (text or "").strip()
    if not value:
        return ""
    value = value.splitlines()[0].strip()
    value = re.sub(r"^(推荐了|推荐吃|推荐|建议|可以吃|就吃|试试吃)?[：:\s]*", "", value)
    value = value.strip(" \t\r\n-—*•「」『』【】。，.!！?？")
...
class FoodPlugin(AmadeusPlugin):
    ...
    def register_commands(self) -> list:
        return [
            ...
            Command(
                ...
                hidden=True,  # 父命令占位，不显示在自身 help 中
                sub_commands=[
                    Command(
                        name="help",
                    ...
                    Command(
                        ...
                        usage="/food like <食物/口味>  例如 /food like 辣的",
                        aliases=["喜欢", "爱吃"],
                        require_args=True,
                    ),
                    Command(
                        ...
                        usage="/food dislike <食物/口味>  例如 /food dislike 香菜",
                        aliases=["不喜欢", "不吃", "讨厌"],
                        require_args=True,
                    ),
                    Command(
                        ...
                        usage="/food location <地区名>  例如 /food location 北京",
                        aliases=["地区", "位置", "在哪"],
                        require_args=True,
                    ),
                    Command(
    ...
    async def _handle_eat(self, cmd_ctx: Any) -> None:
        ...
        if reply is None:
            await self._send_reply(cmd_ctx, "脑袋空空了…等会儿再问我吧")
            return

        await self._send_reply(cmd_ctx, reply)

        food_name = _extract_food_name(reply)
        if food_name:
            await self._record_served_safe(user_id, food_name)

        # Set feedback window so follow-up messages are intercepted
        if cmd_ctx.group_id and food_name:
        ...
        try:
            ...
            if reply is None:
                await bot.send_group_msg(
                    ...
                    message=Message(f"{prefix}脑袋空空了…等会儿再问我吧"),
                )
                return
    ...
    async def _do_recommend(
        ...
        try:
            ...
            text = self._local_recommend_fallback(fallback_items, user_id, period, requirements, user_text)
            selection_mode = "llm_fallback_local"
        except Exception:
        ...
        _L.info("recommend selected | mode={} user={} food={!r}", selection_mode, user_id, food_name)
        return text

    def _local_recommend_fallback(
        self,
        ...
        """Pick a deterministic local-library recommendation when the LLM is unavailable."""
        if not items:
            _L.warning("recommend local fallback unavailable | user={} reason=no_items", user_id)
            return None
        candidates = sorted(
        ...
        if not candidates:
            _L.warning("recommend local fallback unavailable | user={} reason=no_named_items", user_id)
            return None
        seed = f"{user_id}|{period}|{requirements}|{user_text}"
        index = int(hashlib.sha256(seed.encode("utf-8")).hexdigest()[:8], 16) % len(candidates)
        food_name = candidates[index]["name"].strip()
        _L.warning("recommend selected | mode=llm_fallback_local user={} food={!r}", user_id, food_name)
        return food_name
    ...
    async def _handle_like(self, cmd_ctx: Any) -> None:
        ...
        if cmd_ctx.is_private:
            ...
            await self._send_reply(cmd_ctx, "已记录，试试 /吃什么 获取推荐吧")
    ...
    async def _handle_location(self, cmd_ctx: Any) -> None:
        ...
        store = cmd_ctx.plugin_ctx.card_store
        if store is None:
            await self._send_reply(cmd_ctx, "记忆系统未就绪")
            return
    ...
    async def _handle_food(self, cmd_ctx: Any) -> None:
        """Fallback handler — show help when /food has no args."""
    ...
    async def _handle_food_help(self, cmd_ctx: Any) -> None:
        ...
        await self._send_reply(cmd_ctx, help_text)
    ...
    @staticmethod
    def _group_reply_prefix(user_id: str, message_id: int) -> str:
        """CQ reply + @ prefix so group replies don't mix up across users."""
        return f"[CQ:reply,id={message_id}][CQ:at,qq={user_id}] "

    async def _send_reply(self, cmd_ctx: Any, text: str) -> None:
        """Send a reply, prefixing with CQ reply + @ in group context."""
        from nonebot.adapters.onebot.v11 import Message

        if cmd_ctx.group_id:
            text = self._group_reply_prefix(cmd_ctx.user_id, cmd_ctx.event.message_id) + text
    ...
    async def _add_preference(self, user_id: str, pref_type: str, value: str) -> None:
        ...
        if existing is not None:
            ...
            series = await store.get_or_create_series(
                f"food_pref:{user_id}", scope="user", scope_id=user_id,
    ...
    async def _record_served(self, user_id: str, food_name: str) -> None:
        ...
        ctx = self._ctx
        if ctx is None or ctx.card_store is None:
            _L.debug("record served skipped | user={} reason=no_card_store", user_id)
            return
        food_name = _extract_food_name(food_name)
        if not food_name:
            _L.debug("record served skipped | user={} reason=empty_food", user_id)
            return

        store = ctx.card_store
        series_key = f"food_served:{user_id}"

        series = await store.get_or_create_series(
            series_key, scope="user", scope_id=user_id,
            label="食物推荐记录", source="food_plugin",
        )

        existing_cards = await store.get_series_cards(series.series_id)
        for c in existing_cards:
            if food_name in c.content:
                await store.mark_seen(c.card_id)
                _L.debug("record served skipped | user={} food={!r} reason=duplicate", user_id, food_name)
                return

        if len(existing_cards) >= 20:
            await store.expire_card(existing_cards[-1].card_id)
    ...
    async def _record_served_safe(self, user_id: str, food_name: str) -> None:
        try:
            ...
            _L.warning("record served failed | user={} food={!r}", user_id, food_name, exc_info=True)
