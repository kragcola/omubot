...
_STICKER_FREQUENCY_PROMPTS: dict[str, str] = {
    "rarely": (
        ...
        "必须同时调用 send_sticker 发送一个合适且近期未重复的表情包。这不是可选的——颜文字=表情包。\n\n"
        "【表情包触发模式：逐条评估】\n"
    ...
    "frequently": (
        ...
        "必须同时调用 send_sticker，选择合适且近期未重复的表情包。不发就是事故，没有例外。\n\n"
        "不要想太多，看到合适的就发，看不到也随便挑一个发。\n"
        "优先从推荐候选里挑，不要把同一张当默认万能图。"
    ),
}
    ...
    async def on_message(self, ctx: MessageContext) -> bool:
        ...
        for seg in segments:
            ...
            try:
                ...
                _L.warning("silent sticker learn failed | group={} file={}", ctx.group_id, file_id, exc_info=True)
                continue
            if is_new:
                saved += 1
                _L.info(
