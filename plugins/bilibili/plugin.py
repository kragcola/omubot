...
# Standalone av identifiers only: avoid matching noise like "...save4me..."
_AV_PATTERN = re.compile(r"(?<![0-9A-Za-z])(?:av|AV)(\d+)(?![0-9A-Za-z])")
...
class BilibiliPlugin(AmadeusPlugin):
    ...
    async def on_startup(self, ctx: PluginContext) -> None:
        ...
        cfg = load_plugin_config("plugins/bilibili/config.default.json", BilibiliConfig)
    ...
    async def on_message(self, ctx: MessageContext) -> bool:
        ...
        # Prefer structured QQ mini-program cards before generic free-text scanning.
        if json_info:
            for card_url in json_info.get("_all_urls", []):
                ...
                if resolved:
                    ...
                    source = "json_url"
                    _log.info(
                        "bilibili | source={} url resolved | url={!r} -> vid={}",
                        source, card_url[:80], vid.key,
                    )
                    break

            if vid is None:
                _log.info("bilibili | source=json_search fallback | title={!r}", json_info["title"])
                info = await self._search_video(json_info["title"])
                if info:
                    source = "json_search"
                    _log.info(
                        "bilibili | source={} resolved | title={!r} -> bvid={}",
                        source, json_info["title"], info.get("bvid", "?"),
                    )

        # Try direct URL extraction from plain text / safe extracted card text
        ...
        if vid is not None and info is None:
            _log.info(
                "bilibili | group={} user={} vid={} source={}",
                ctx.group_id, ctx.user_id, vid_key, source or "unknown",
            )
            try:
                info = await self._get_video_info(vid)
            except Exception:
                _log.warning("bilibili | vid={} source={} video info fetch failed", vid_key, source or "unknown")
                return False
        ...
        if self._reply_mode != "mood":
            ...
            ctx.raw_message["_bilibili_reply"] = hint  # kept for backward compat during migration
            from kernel.types import TriggerContext
            ctx.trigger = TriggerContext(
                reason=f"视频分享:《{title}》",
                mode=f"video_{self._reply_mode}",
                target_message_id=ctx.message_id,
    ...
    def _collect_segment_text(ctx: MessageContext) -> str:
        """Collect low-noise searchable text from message segments."""
        parts: list[str] = []
        ...
        for seg in segments:
            ...
            if seg_type == "json" and isinstance(seg_data, dict):
                ...
                if isinstance(raw, str):
                    ...
                    extracted = _extract_json_card_text(raw)
                    if extracted:
    ...
    async def _get_video_info(self, vid: _VideoId) -> dict | None:
        ...
        try:
            ...
            _log.warning(
                "bilibili | API call failed vid={} error_type={} error={!r}",
                vid_key, type(e).__name__, str(e)[:200],
            )
            return None
