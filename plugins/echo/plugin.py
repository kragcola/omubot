...
class EchoTracker:
    ...
    def __init__(
        ...
        *,
        window_seconds: float = _ECHO_WINDOW_S,
        threshold: int = _ECHO_THRESHOLD,
        break_text: str = _BREAK_TEXT,
        break_chance: float = _BREAK_CHANCE,
    ) -> None:
        self._states: dict[str, _EchoState] = {}
        self._rand = rand or random.random
        self._window_seconds = max(1.0, float(window_seconds))
        self._threshold = max(2, int(threshold))
        self._break_text = break_text or _BREAK_TEXT
        self._break_chance = min(1.0, max(0.0, float(break_chance)))
    ...
    def process(self, group_id: str, text: str, now: float) -> str | None:
        ...
        if self._rand() < self._break_chance:
            state.interrupt_chain = 0
...
class EchoPlugin(AmadeusPlugin):
    ...
    async def on_startup(self, ctx: PluginContext) -> None:
        self._config = load_plugin_config("plugins/echo/config.default.json", EchoConfig)
        self._tracker = EchoTracker(
            window_seconds=self._config.window_seconds,
            threshold=self._config.threshold,
            break_text=self._config.break_text,
            break_chance=self._config.break_chance,
        )
    ...
    async def on_message(self, ctx: MessageContext) -> bool:
        ...
        if self._config.ignore_command_messages and plain_text.startswith("/"):
            return False
